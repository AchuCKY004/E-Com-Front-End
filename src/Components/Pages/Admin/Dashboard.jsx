...
const DashboardHome = () => {
    ...
    const fetchDashboardData = async () => {
        try {
            setLoading(true);
            // Use your specific analytics endpoint
            const response = await Api.get("/admin/analytics/dashboard");
            const data = response.data;

            // Mapping the API response to our state
            // Total revenue is nested in an array based on your JSON
            const revenueValue = data.totalRevenue?.[0]?.total || 0;

            setStats({
                revenue: revenueValue,
                orders: data.totalRevenue?.[0]?.totalOrders || 0,
    ...
    if (loading) {
        return <div className="p-5 text-center">Loading Analytics...</div>;
    }
                ...
                <div className="col-md-8">
                    <div className="card border-0 shadow-sm p-4" style={{ minHeight: '300px' }}>
                        ...
                        <div className="d-flex flex-column align-items-center justify-content-center mt-5">
                            <p className="text-muted mb-0">Total Business Value</p>
                            <h2 className="text-primary fw-bold">₹{stats.revenue.toLocaleString()}</h2>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="card border-0 shadow-sm p-4" style={{ minHeight: '300px' }}>
                        ...
                        <div className="py-2">
                            <div className="d-flex justify-content-between mb-2">
                                <span className="text-muted">Products in Catalog</span>
                                <span className="fw-bold">{stats.products}</span>
                            </div>
