...
// import { FaMapMarkerAlt, FaPlus, FaCheckCircle, FaHome, FaInfoCircle , FaEdit , FaTrash } from 'react-icons/fa';
...
//         try {
//             // NOTE: Ensure your backend has router.get('/', ...) inside the address router
//             const res = await Api.get('/address');

//             // Matching your JSON: { address: { addressess: [...] } }
//             const list = res.data.address?.addressess || [];
//             setAddresses(list);

//             // Logic to auto-select primary or first address
//             if (list.length > 0 && typeof setSelectedAddressId === 'function') {
//                 const primary = list.find(a => a.isPrimary) || list[0];
//                 setSelectedAddressId(primary._id);
//             }
//         } catch (err) {
//             console.error("Fetch Error:", err);
//             // If 404 persists, the route /api/address is not defined on the backend
//             if (err.response?.status === 404) toast.error("Address API not found (404)");
//         }
//     };
...
//     const handleDelete = async (addressId) => { // Changed param name for clarity
...
//                                 <button type="submit" className="btn btn-primary px-4" disabled={loading}>{loading ? "Saving..." : "Save Address"}</button>
...
//                                     </div>
//                                     {selectedAddressId === item._id && <FaCheckCircle className="text-primary h4" />}
//                                 </div>
//                             </div>
//                         </div>
//                     )) : (
//                         <div className="col-12 text-center py-5 border rounded bg-light border-dashed">
//                             <p className="text-muted mb-0">No saved addresses found. Please add a new one.</p>
//                         </div>
//                     )}
//                 </div>
//             </div>
